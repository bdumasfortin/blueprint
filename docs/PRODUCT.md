...
Status: interaction and minimum-core architecture contracts approved for local implementation and user testing on 2026-08-28; AXI distribution direction, its first local implementation slice, and the approval/revision lifecycle approved on 2026-08-29; pending-revision approval gating, atomic launch-and-wait, specialist positioning, MIT licensing direction, and a validation-gated npm publication path approved on 2026-08-30; typed decision-response semantics and removal of the generated agent skill approved on 2026-09-02; review-specific browser tab identity approved on 2026-09-03. This document records approved directions and explicitly separates them from testing hypotheses and deferred product choices.
...
5. The final action is **Approve** unless at least one actual revision comment is queued. A queued decision response does not change that label or expose a revision action. With revision feedback, the action becomes **Approve with feedback** and **Revise using feedback** appears beside it.
...
- Blueprint is formally an AXI. The approved distribution direction is a global npm CLI with explicit, reversible SessionStart hooks for Codex and Claude Code plus a later `npx` evaluation path. The CLI playbooks and optional hook own agent guidance; the npm package does not ship an agent skill beginning with 0.2.1.
...
- Comments are private local drafts until the reviewer deliberately chooses **Approve with feedback** or **Revise using feedback**. Decision responses are separate typed private inputs and do not count as comments.
...
- The default structure is an **Ask-first brief**. Put the human-readable project or workspace name before the review title; include a filesystem path only when the readable name is ambiguous.
- The opening viewport leads with one recommendation and one reason, then previews every decision, preference, or confirmation the review will request. Link each preview item to its response area when the artifact scrolls. The reviewer must not need to reach the end to discover the ask.
- Keep response controls beside the smallest amount of evidence needed to choose. Do not move all controls to the top or add a persistent decision rail by default.
- Collapse supporting evidence and edge cases by default unless they are necessary to understand the primary options. Visible content must help the reviewer orient, decide, or verify; concise labels and short comparable tradeoffs are preferred over repeated summaries.
- Omit background and rationale already established in the conversation unless it materially changes the recommendation or a choice. Carry settled decisions forward without replaying the thread or asking for them again.
- This is an authoring contract for generated HTML and agent guidance. It does not authorize runtime rewriting of authored content, new workspace persistence, or a new review-chrome feature.
...
- Selection is reversible browser-local state, not feedback or authorization. A separate **Queue response** action serializes the form's meaningful named values into one editable, typed private decision response. It appears under a dedicated **Decision responses** heading rather than in the private-feedback group. Re-queueing the same form before delivery replaces that response in place instead of adding duplicates.
- Queue response never contacts the agent, approves the artifact, requests a revision, or ends the session. A queued decision response alone leaves the final action labelled **Approve** and never exposes **Revise using feedback**. Approval delivers the decision and closes the review. When actual revision comments are also queued, a revise submission may carry both typed decision inputs and comments, but only the comments create amendment items.
- Packet schema version 3 carries `decisions` and `comments` as separate collections. Decisions are durable basis inputs in History; they never receive amended-element markers, amendment reports, or Accept/Reopen controls.
...
- Each review browser tab uses the visible immutable revision's HTML `<title>` followed by `· Blueprint`, displays the approved **Review aperture** favicon, and repeats the readable review name in the inspector. Titleless artifacts and older session revisions fall back to the HTML filename. A newly staged title remains hidden until the reviewer reveals that revision. The favicon uses four cyan focus corners around one light reviewed point on graphite; the rejected grid favicon is not part of the product.
...
- Successful approval presents **View approved review** as the sole completion action. It opens the immutable approved snapshot and History in the same tab, visibly marked read-only, with Feedback and every mutation control absent. **Back to completion** returns to that action. Reload returns to the completion screen rather than preserving this transient viewing choice. Blueprint does not offer scripted tab closing because Chrome refuses it for tabs the page did not open.
- The draft pane shows one concise instruction—“Hold Alt/Option and click any element to leave feedback.”—does not repeat it as an empty state, and keeps an unlabeled **Additional feedback** composer aligned immediately above the bottom action bar. Queuing that composer creates a normal general comment with a stable identity rather than a packet-level note. Additional feedback has no manual resize grip and grows vertically to fit its content. Quiet polling and autosave must not move a long Feedback pane away from the focused composer; a genuine remote update preserves its scroll anchor, field focus, and text selection while incorporating the new state. Draft editors use the approved compact edge-rail treatment: one content-sized line at rest, automatic vertical growth only when text wraps, no manual resize grip, and a red borderless trash-can delete action whose full-size target stays vertically centered as the comment grows. The edge rail is the editing-state signal; focusing the text itself does not add another rectangular outline. The end-review stop glyph remains compact but visually prominent.
- The inspector has **Feedback** and **History** tabs. Feedback is the sole action surface, but its queued content is explicitly separated into **Decision responses** and **Private feedback**. It also contains the persistent revision-request delivery state and every submitted non-accepted comment. An element-anchored draft label or submitted-feedback card is navigable: activating it scrolls the visible artifact to the associated element and briefly outlines that target, preferring the visible revision's reported amendment selector when available. Sent comments remain visible while waiting for or being worked by the agent; Accept/Reopen appears only after the agent reports an amendment. Accepted items disappear from Feedback. History is a read-only revision-cycle ledger of revealed snapshots, submitted decision inputs, submitted comments, amendment evidence, and current reviewer state; it never exposes staged revision evidence before reveal.
...
- **Approve** sends a final intent-bearing batch and ends the review, with or without decisions or comments. A decision-only approval remains labelled **Approve**. After successful persistence, the shell stops polling and accepting annotation input, blanks the artifact iframe, hides the review application, and shows a terminal completion screen. Its sole action projects the exact approved snapshot plus immutable History back into the same shell as read-only; this projection loads without Blueprint's annotation runtime and never reactivates the ended session. The queued final packet remains available to the attached agent wait. **Revise using feedback** appears only when there is at least one valid revision comment; a decision response alone can never create a revise packet. A revise packet may also carry queued decisions as separately typed basis inputs, clears the submitted decisions and comments, and keeps the session open for additional batches. The approval control is disabled, and the reviewer endpoint rejects approval, until every submitted revision request is represented by a revision the reviewer has revealed.
...
- Revision reports account for comments only. Decisions carried by the same basis packets are recorded in History but do not participate in amendment or acceptance lifecycle.
...
For this slice, packet delivery is at least once and duplicate-safe by packet ID; a packet remains queued until the CLI has fully written it and acknowledged it. Packet schema version 3 separates `decisions` from `comments`, while older packet schemas remain readable. The atomic `review` command opens and waits in one process, writes launch diagnostics to standard error, and reserves standard output for one exact packet. The initial snapshot is visible immediately after the requested launch. Approval ends the session, **Revise using feedback** requires at least one comment and keeps the session active, and staging never changes the visible revision. Approval is rejected while any revise packet lacks a revealed basis revision or while any staged revision is unrevealed. The blocking reveal action, accept, reopen, approval, and end-session remain reviewer-only actions.
...
On 2026-09-02 the user approved the recommended post-approval completion path: **Close tab** plus a same-tab **View approved review** action. The latter is an immutable, History-visible read-only projection of the ended session, never a return to drafting or review authority. The approval covers the scoped local runtime, tests, documentation, and agent guidance only; it does not authorize a commit, release, publication, packaging, installation, or other external effect.

Later on 2026-09-02 the user explicitly removed the generated Blueprint agent skill from the product and npm package. The CLI's versioned playbooks and optional SessionStart hook are the sole agent-guidance surfaces beginning with 0.2.1. Blueprint does not manage or remove manually installed legacy skill copies; affected users are instructed to uninstall them. This approval covers the local source, tests, package boundary, release notes, and durable documentation, but not changes to any active agent installation, commit, push, or publication.

The user then separately authorized removal of the Blueprint skill from this computer's personal Codex configuration. That removal does not alter the SessionStart hook, global package installation, repository history, or other agent configuration.

On 2026-09-03 the user approved the focused review-tab identity contract without feedback: use the visible revision's decoded artifact title as the review name, fall back to the filename, append `· Blueprint` in the browser tab, reuse the name in the inspector, and add one stable embedded Blueprint favicon. The approval authorizes local code, tests, and durable documentation only; publication, versioning, Git operations, and installed-package replacement remain separate authority gates.

The subsequent local-source Chrome check confirmed that the dynamic title and favicon were visible, but the user rejected the initial grid icon and requested a new Blueprint design review. That fresh review selected and approved the recommended **Review aperture** direction: four cyan focus corners around one light reviewed point on graphite. The rejected grid must not return. This approval authorizes the chosen local icon replacement, tests, and durable notes only; release and installation remain separate.

A final local-source Chrome QA review then exercised the selected Review aperture favicon in the trusted shell together with the dynamic `Blueprint tab identity QA · Blueprint` title. The user approved the result without feedback.

The user then explicitly removed **Close tab** from the post-approval completion screen after direct Chrome testing showed that scripted closing does not work for these user-opened review tabs. **View approved review** remains as the sole completion action; the failed close attempt and its manual-close fallback are removed. This authorizes local code, tests, and durable documentation only.

The user subsequently requested a plain closing hint in the completion dialog. Approved and otherwise ended reviews now say **You can close this tab**, including when an ended session is loaded again. This is informational copy, not a restored Close tab button or scripted close attempt.

On 2026-09-03 the user identified two recurring content failures: multiple simultaneous reviews lack immediate project/workspace identity, and superfluous background hides the decisions until the end. A fresh three-specimen Blueprint review approved the recommended **Ask-first brief** plus all four defaults: workspace before title, every ask in the opening viewport, supporting evidence and edge cases collapsed by default, and established conversation context omitted unless it changes the decision. The approval authorizes local playbook, design-guidance, documentation, representative-example, and test changes only. It does not authorize runtime redesign, commit, push, release, publication, or installation.

The user subsequently authorized committing and pushing the completed Ask-first changes and completion-dialog hint on the current branch. Release, publication, and installation remain separate authority gates.

The user then explicitly authorized preparing, publishing, and installing version 0.2.2, with all approved changes integrated into `main`. The release follows the existing exact-tarball validation, CI, registry-integrity verification, and local-installation checks; existing agent configuration is preserved.

The 0.2.2 sequence completed from source commit `78cb278` on `main`: all four CI jobs and exact-package browser/CLI checks passed, the final Blueprint release review approved the candidate, npm published the unchanged tested tarball as `latest`, and clean registry/global installations matched every packaged file. The user separately authorized restarting the local service despite four active sessions; it retained the same address and agent capability, and all four sessions remained available. The installed service passed authenticated health and served the new completion text without changing Codex or Claude configuration.
