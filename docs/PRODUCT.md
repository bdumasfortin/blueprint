...
- Successful approval presents **Close tab** and **View approved review**. The latter opens the immutable approved snapshot and History in the same tab, visibly marked read-only, with Feedback and every mutation control absent. **Back to completion** returns to the two approval-completion actions. Reload returns to the completion screen rather than preserving this transient viewing choice. Closing is best effort because browsers may refuse to close a tab the page did not open; Blueprint then asks the reviewer to close it manually.
