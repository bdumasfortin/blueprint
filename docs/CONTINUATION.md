...
- The workspace is a Git repository on branch `main`; `blueprint-local-review@0.2.0` is the current public and locally installed AXI milestone.
...
- The MIT-licensed `blueprint-local-review@0.2.0` package is publicly available from npm with `latest` pointing to `0.2.0`. The registry integrity and SHA-1 match the exact reviewed tarball. This computer now has the public 0.2.0 package installed globally; npm installation did not edit agent configuration, the existing Codex integration still reports installed without repair, and the active personal Blueprint skill is byte-identical to both the repository and installed-package copies.
...
- Blueprint playbook version 7 requires operable decision controls when a reviewer is expected to choose: radios for mutually exclusive options, checkboxes or switches for independent choices, and selects for longer lists. Decision-relevant UI specimens must expose useful states, transitions, or microinteractions instead of relying on static wireframes. Its review-loop guidance makes atomic `review` the normal launch, reserves separate `open` and `wait` for recovery, requires the attached turn to remain active, keeps sent feedback visible with acknowledgement-driven waiting/working state while the agent prepares a revision, keeps final approval unavailable until the requested revision is revealed, and retires the browser review surface after final approval.
...
Use real personal review sessions to test whether Blueprint's specialist workflow creates enough value over Lavish to justify its extra ceremony. Treat 0.2.0 as the first public evidence-gathering release, not as a claim of broad feature parity or production maturity. The matching source is recorded by tag `v0.2.0`; future package changes require a new version rather than repacking this release.
