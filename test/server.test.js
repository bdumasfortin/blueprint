...
test("agent and reviewer authorities are separated and artifact routes are sandboxed", async (t) => {
  ...
  assert.match(shell, /The review session is closed\. Your final feedback is queued for the agent/);
  ...
  assert.match(shell, /Feedback copied/);
  ...
  assert.match(shell, /noteField\.addEventListener\("keydown", queueAdditionalFeedbackOnEnter\)/);
  ...
  assert.match(shell, /id="feedback-pane"[\s\S]*id="draft-list"[\s\S]*id="review-items"[\s\S]*id="feedback-list"[\s\S]*id="packet-note"/);
  ...
  assert.match(shell, /id="history-pane"[\s\S]*Read-only history of revealed revisions, submitted comments, and reported amendments/);
  ...
  assert.match(shell, /\.packet-note \{[^}]*height: 70px;[^}]*min-height: 70px;[^}]*resize: none;[^}]*overflow: hidden/);
  assert.match(shell, /function sizePacketNote\(\) \{[\s\S]*?noteField\.style\.height = "0px";[\s\S]*?Math\.max\(70, noteField\.scrollHeight\)/);
  assert.match(shell, /noteField\.addEventListener\("input", \(\) => \{ sizePacketNote\(\)/);
  ...
  assert.match(artifactHtml, /Queued in Feedback\. Use Revise using feedback or Approve with feedback to send/);
